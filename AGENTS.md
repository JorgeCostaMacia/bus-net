...
- **Quartz 4** (`Quartz*` 4.0.1) — this upgrade is what forced the single `net10.0` target: **Quartz 4 ships only `lib/net10.0`** (3.20 shipped net462/net472/netstandard2.0/net8/net9/net10), so multi-targeting and Quartz 4 are mutually exclusive. Its breaking changes as they land here: `IJob.Execute` takes a `CancellationToken` and returns `ValueTask`; `ScheduleJob` / `AddJob` replaced the `replace` bool with `ScheduleJobOptions` / `AddJobOptions` (`…Options.Replacing`); the matchers moved from `Quartz.Impl.Matchers` to `Quartz`; `StdSchedulerFactory` is gone, replaced by `QuartzSchedulerBuilder.Create().UseProperties(…).Build()`; `IPersistentStoreBuilder.UseProperties` became `ConfigureStore(options => options.StoreJobDataAsStrings = true)`; and `IJobExecutionContext` lost `Get` / `Put` (use `MergedJobDataMap`) and gained `RetryAttempt`.
- **The Quartz 4 schema is validated on start and is not the 3.x one** — it adds `QRTZ_PAUSED_JOB_GRPS` and new columns (`RETRY_POLICY`, `RETRY_ATTEMPT`, `PREFERRED_NODE`), so a 3.x schema fails `ValidateSchema` at startup. Quartz 4 embeds the DDL per provider, so the integration fixtures no longer carry a hand-written copy of `tables_postgres.sql`: the test store calls **`store.ProvisionSchema()`** and Quartz creates whatever is missing. It only ever *creates* — it is not an upgrade, so a **live** 3.x database still needs `database/migrations/4.0/schema_30_to_40_upgrade_postgres.sql` run before the fleet starts on Quartz 4.
...
Tests are **xUnit v4 on Microsoft.Testing.Platform v2** — test projects are `OutputType=Exe`, and `dotnet test` runs MTP because the root **`global.json`** opts in (`"test": { "runner": "Microsoft.Testing.Platform" }`). MTP v2 dropped the VSTest bridge, so `TestingPlatformDotnetTestSupport` is gone and running the tests needs the **.NET 10 SDK**. The integration suites (Kafka / RabbitMQ / Postgres via Testcontainers) are the slow ones: running the whole solution starts the four of them at once (one per project, now that the repo single-targets) and Docker contention can make them flake — re-run a failing suite on its own (its MTP `.exe` under `bin/Release/net10.0/`) before treating it as a real failure.
