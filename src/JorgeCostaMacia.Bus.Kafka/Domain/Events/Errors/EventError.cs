    ...
    where TEvent : Event
{
    ...
    public EventError(string errorType, string errorMessage, string? errorStackTrace, DateTime errorOccurredAt, int retryCount, string groupId, string topic, int partition, long offset, DateTime timestamp, ImmutableList<KeyValuePair<string, string>> headers, TEvent message)
    {
        ErrorType = errorType;
