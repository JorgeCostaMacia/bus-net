...
/// notify, decide), invoked when the command's handler fails terminally and the delivery parks to its
/// error queue: closes the <see cref="IHandler{TMessage, TContext}"/> contract over
/// <see cref="CommandErrorContext{TCommand}"/>, so a concrete error handler declares only its command
