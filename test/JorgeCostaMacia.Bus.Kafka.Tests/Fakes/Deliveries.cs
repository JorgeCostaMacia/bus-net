...
internal static class Deliveries
{
    ...
    public static ConsumeResult<Ignore, byte[]> Garbage(long offset = 10) => Result("}{ not json"u8.ToArray(), new Headers(), offset);

    /// <summary>A delivery whose body deserializes to <see langword="null"/> — drives the null-body/fault path.</summary>
    public static ConsumeResult<Ignore, byte[]> NullBody(long offset = 10) => Result("null"u8.ToArray(), new Headers(), offset);

    /// <summary>A tombstone — a delivery with no body at all (<c>Value = null</c>) — drives the fault path without crashing it.</summary>
    public static ConsumeResult<Ignore, byte[]> Tombstone(long offset = 10) => Result(null!, new Headers(), offset);
    ...
    public static ConsumeResult<Ignore, byte[]> MissingTrace<TMessage>(TMessage message, long offset = 10)
        => Result(JsonSerializer.SerializeToUtf8Bytes(message), new Headers(), offset);

    /// <summary>A transport over an envelope with NO trace headers — for the error handlers' unreadable-envelope path.</summary>
    public static Transport BareTransport() => KafkaTransport.Create(Result("{}"u8.ToArray(), new Headers()));
