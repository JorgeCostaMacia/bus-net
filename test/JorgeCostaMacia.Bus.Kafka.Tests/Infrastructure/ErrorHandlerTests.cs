...
public class ErrorHandlerTests
{
    ...
    private Infrastructure.Consumer.Commands.CommandErrorHandler<TestCommand> CommandError(ImmutableList<TimeSpan>? intervals = null, ImmutableList<Type>? excludes = null, bool scheduler = true)
        => new(Bus(), scheduler ? _scheduler : null, NullLogger.Instance, TOPIC, GROUP_ID, intervals ?? [], excludes ?? []);

    private Infrastructure.Consumer.Events.EventErrorHandler<TestEvent> EventError(ImmutableList<TimeSpan>? intervals = null)
        => new(Bus(), _scheduler, NullLogger.Instance, TOPIC, GROUP_ID, intervals ?? [], []);

    private Infrastructure.Consumer.Faults.FaultHandler Fault() => new(Bus(), NullLogger.Instance, TOPIC, GROUP_ID);
    ...
    public async Task Command_NoLadder_ParksToErrorTopic()
    {
        ...
        Infrastructure.Consumer.Commands.CommandErrorHandler<TestCommand> sut = CommandError();
    ...
    {
        Infrastructure.Consumer.Commands.CommandErrorHandler<TestCommand> sut = CommandError([TimeSpan.Zero]);
    ...
    {
        Infrastructure.Consumer.Commands.CommandErrorHandler<TestCommand> sut = CommandError([TimeSpan.FromMinutes(5)]);
    ...
    {
        Infrastructure.Consumer.Commands.CommandErrorHandler<TestCommand> sut = CommandError([TimeSpan.Zero, TimeSpan.Zero]);
    ...
    {
        Infrastructure.Consumer.Commands.CommandErrorHandler<TestCommand> sut = CommandError([TimeSpan.Zero], [typeof(BaseFailure)]);
    ...
    {
        Infrastructure.Consumer.Commands.CommandErrorHandler<TestCommand> sut = CommandError([TimeSpan.FromMinutes(5)], scheduler: false);
    ...
    {
        Infrastructure.Consumer.Commands.CommandErrorHandler<TestCommand> sut = CommandError([TimeSpan.Zero, TimeSpan.Zero, TimeSpan.Zero]);
    ...
    {
        Infrastructure.Consumer.Events.EventErrorHandler<TestEvent> sut = EventError([TimeSpan.Zero]);
    ...
    public async Task Fault_ParksToFaultTopic_WithTheBodyAsText()
    {
        ...
        Infrastructure.Consumer.Faults.FaultHandler sut = Fault();
        await sut.Handle(context, TestContext.Current.CancellationToken);
