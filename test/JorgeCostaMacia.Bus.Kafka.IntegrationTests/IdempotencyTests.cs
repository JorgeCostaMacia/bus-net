...
/// sent twice, modelling the duplicate delivery the bus can produce (a redelivery after a lost
/// commit/ack, as the broker-outage test shows happening for real). An idempotent handler dedups by the
/// record's id, so although <see cref="Records"/> × 2 deliveries arrive, exactly <see cref="Records"/>
/// effects are applied: at-least-once + dedup = effectively-once. Guards the pattern the ETL depends on
/// to absorb the duplicates the bus is allowed to deliver.
/// </summary>
[Trait("Category", "Integration")]
public sealed class IdempotencyTests : IClassFixture<KafkaFixture>
{
    private const string Topic = "idempotency-integration-tests";
    private const string GroupId = "idempotency-integration-tests.handler";
    ...
    public IdempotencyTests(KafkaFixture fixture, ITestOutputHelper output)
    {
        _fixture = fixture;
        _output = output;
    }
    ...
    public async Task Send_everyRecordTwice_AppliesEachEffectExactlyOnceUnderAnIdempotentHandler()
    {
        ...
        builder.Services.AddBusContext(
            ...
            consumer => consumer.AddCommandHandler<ChaosCommand, IdempotentCommandHandler>(GroupId));
        ...
        try
        {
            using (IServiceScope scope = host.Services.CreateScope())
            {
                ...
                // at-least-once contract permits (the bus re-handing a record whose commit was lost).
            ...
            _output.WriteLine($"Kafka idempotency: {probe.Received} deliveries collapsed to {probe.Effects} effects ({Records} distinct records sent twice).");
        }
        finally
        {
