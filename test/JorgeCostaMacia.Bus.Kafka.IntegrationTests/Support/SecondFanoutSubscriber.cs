...
/// <summary>
/// The second subscriber to <see cref="FanoutEvent"/> on its own separate consumer group: signals the
/// payload it received on the shared <see cref="FanoutProbe"/>, so the test can prove a second group,
/// reading the same topic, got its own copy of the one published event.
/// </summary>
